...
Oltre a ridurre la risoluzione rende addestrabile il campionamento sparso: il
gradiente sulle coordinate e' una differenza finita fra celle adiacenti, e su
uno spettrogramma grezzo sarebbe troppo rumorosa (paper, sez. 3.2).

Sequenza come nel codice di SparseFormer (imagenet/models/sparseformer.py):
conv 7x7 stride 2 pad 3 -> ReLU -> maxpool 3x3 -> LayerNorm(C).

Il paper dice "96 dimensional feature" e due righe dopo "196 kernels".
`channel_reading` espone le due letture; si adotta c96, che e' quella di
...
class ChannelLayerNorm(nn.Module):
    ...
    spaziale rispetto ai propri canali, non sul batch come BatchNorm2d.
    """
...
class EarlyConv(nn.Module):
    ...
    def __init__(
        ...
        [ASSUNZIONE] (2, 1) dimezza la sola frequenza e lascia 51 frame; (2, 2)
        li ridurrebbe a 26, cioe' un passo di 40 ms. Il paper dice solo "max
        pooling", senza stride; SparseFormer usa 2 su entrambi gli assi, che su
        un'immagine e' naturale perche' le due dimensioni sono omogenee.
        """
        super().__init__()
        ...
        # un solo strato convolutivo, come SparseFormer: il plurale del paper
        # ("convolutional layers") descrive la sequenza conv+ReLU+pool
